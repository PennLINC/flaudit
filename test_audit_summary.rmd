...
And hence, the missingness of sequences across all the subjects in the project:
```{r, echo=FALSE}
...
gear_nodes <- jobs %>%
  group_by(gear_name) %>%
  summarise(n = n()) %>%
  create_node_df(n = nrow(.), label = .$gear_name, data = .$n, type = 'lower', style = "filled")
...
for(x in 1:nrow(gear_edges)){
  
  
  gear_edges$from[x] <- filter(gear_nodes, label == gear_edges$from[x]) %>%
    pull(id)
  gear_edges$to[x] <- filter(gear_nodes, label == gear_edges$to[x]) %>%
    pull(id)

}

gear_edges <- mutate(gear_edges,
                     from = as.numeric(from),
                     to = as.numeric(to))

create_graph(nodes_df = gear_nodes, edges_df = gear_edges) %>%
  render_graph()
```
